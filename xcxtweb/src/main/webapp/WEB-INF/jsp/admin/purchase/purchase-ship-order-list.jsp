...
<body>
	<div class="pd-20">
		<form action="/admin/purchase/purchaseList.do" method="post" id="form">
			<input type="hidden" id="purchaseType" name="purchaseType" value="shipOrder" />
			<input type="hidden" id="orderDefind" name="orderDefind"
				value="${orderDefind}" />
			...
			<select class="select" name="createType" id="createType" size="1">
				...
				<option value="2" <c:if test="${createType == 2}">selected</c:if>>短信预约</option>
					...
					<input type="text" placeholder="搜索菜品名称" class="input-text ac_input"
						value="${searchKey}" id="searchKey" name="searchKey"
						autocomplete="off" style="width: 200px">
					<input class="input-text ac_input" id="purchaseDate" onClick="WdatePicker()" name="purchaseDate" value="${purchaseDate }" placeholder="采购日期" autocomplete="off" style="width: 200px" />
		...
		<table
			...
			<thead>
				<tr class="text-c">
					...
					<th width="25">#</th>
					<th class="col2">采购日期</th>
					<th class="col2">客户名称</th>
					<th class="col2">菜品</th>
					<th class="col2">规格</th>
					<th class="col2">数量</th>
					<th class="col2">单价</th>
					<th class="col2">合计金额(元)</th>
				</tr>
			</thead>
			<tbody id="dataTable">
			</tbody>
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/purchase/purchaseListAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var purchaser = data.result[i].purchaser == null ? "" : data.result[i].purchaser;
							...
							var foodprice = data.result[i].foodprice == null ? "" : data.result[i].foodprice;
							...
							html += "<td class='col1'>" + orderdate + "</td>";
							html += "<td class='col1'>" + purchaser + "</td>";
							html += "<td class='col1'>" + foodInfoname + "</td>";
							html += "<td class='col1'>" + foodnum + "</td>";
							html += "<td class='col1'>" + foodprice + "</td>";
