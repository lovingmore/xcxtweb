...
<body>
<div class="pd-20">
	<form action="/admin/order/list.do" method="post" id="form">
	...
	<div class="cl pd-5 bg-1 bk-gray">
		...
		<span class="select-box radius" style="width:150px;background-color:#FFFFFF">
			...
			<select class="select" name="orderStatus" id="orderStatus" size="1">
				<option value="-1">全部</option>
				<option value="0" <c:if test="${orderStatus == 0}">selected</c:if>>待支付</option>
				<option value="1" <c:if test="${orderStatus == 1}">selected</c:if>>待发货</option>
				<option value="2" <c:if test="${orderStatus == 2}">selected</c:if>>待收货</option>
				<option value="2" <c:if test="${orderStatus == 3}">selected</c:if>>已结束</option>
			</select>
		</span>
		<input type="text" placeholder="搜索购买人、手机、菜品" class="input-text ac_input" value="${searchKey}" id="searchKey" name="searchKey" autocomplete="off" style="width:200px">
	...
	<thead>
	  <tr class="text-c">
		...
		<th width="25">#</th>
		<th class="col2">订单号</th>
		<th class="col2">下单日期</th>
		<th class="col2">购买人</th>
		<th class="col2">联系方式</th>
		<th class="col2">订单状态</th>
		<th class="col2">付款方式</th>
		<th class="col2">合计</th>
		<th class="col2">操作</th>
	  </tr>
    </thead>
...
<script type="text/javascript">
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/order/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var orderno = data.result[i].orderNo == null ? "" : data.result[i].orderNo;
							...
							var mobile = data.result[i].mobile == null ? "" : data.result[i].mobile;
							var orderstatus = data.result[i].orderStatus == null ? "" : data.result[i].orderStatus;
							var paymenttype = data.result[i].paymentType == null ? "" : data.result[i].paymentType;
							var totalamount = data.result[i].totalAmount == null ? "" : data.result[i].totalAmount;
							if(paymenttype==1){
								statusName="微信支付";
							}else{
								statusName="线下支付";
							...
							html += "<td class='col1'><a href='javascript:Data.edit(\""+id+"\");' >" + orderno + "</a></td>";
							html += "<td class='col1'>" + orderdate + "</td>";
							html += "<td class='col1'>" + purchaser + "</td>";
							html += "<td class='col1'>" + mobile + "</td>";
							html += "<td class='col1'>" + orderstatus + "</td>";
							html += "<td class='col1'>" + paymenttype + "</td>";
							html += "<td class='col1'>" + totalamount + "</td>";
							html += "<td class='col1'>";
...
// 						alert(html);
		...
		"edit":function(id){
			...
			var layerIndex = layer.open({
		      ...
		      area: ['650px', '500px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/order/toEdit.do?id='+id
		    });
		...
		"changeStatus":function(id){
			if(confirm("确定更改状态？")){
				$.ajax({
					url:"/admin/order/changeStatus.do",
					data:{
						"id" : id
					},
					"dataType" : "json",
