...
<body>
	<div class="pd-20">
		<form action="/admin/buyPrice/list.do" method="post" id="form">
			...
			<select class="select" name="createType" id="createType" size="1">
				...
				<option value="2" <c:if test="${createType == 2}">selected</c:if>>短信预约</option>
					...
					<input class="input-text ac_input" id="buyDate" onClick="WdatePicker()" name="buyDate" value="${buyDate }" placeholder="采购日期" autocomplete="off" style="width: 200px" />
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/buyPrice/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var foodName = data.result[i].foodName == null ? "" : data.result[i].foodName;
							var price = data.result[i].price == null ? "" : data.result[i].price;
							var buyDate = data.result[i].buyDate == null ? "" : data.result[i].buyDate;
							if (buyDate != null && buyDate != "") {
								var value = new Date(buyDate);
								buyDate = value.format("yyyy-MM-dd");
							}
							html += "<tr>";
							html += "<td class='col1'>"+(i+1)+"</td>";
							html += "<td class='col1'><a href='javascript:Data.edit(\""+id+"\");' >" + foodName + "</a></td>";
							html += "<td class='col1'>" + price + "</td>";
							html += "<td class='col1'>" + buyDate + "</td>";
							html += "<td class='col1'><a href='javascript:Data.delete(\""+id+"\");' >删除</a></td>";
							html += "</tr>";
						}
// 						alert(html);
						$("#dataTable").html(html);
