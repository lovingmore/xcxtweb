...
<body>
	<div class="pd-20">
		<form action="/admin/foodCategory/list.do" method="post" id="form">
			...
			<select class="select" name="createType" id="createType" size="1">
				...
				<option value="2" <c:if test="${createType == 2}">selected</c:if>>短信预约</option>
					...
					<input type="text" placeholder="搜索菜品分类" class="input-text ac_input"
		...
		<span class="btn btn-primary radius dropDown dropDown_hover fr"> <span class="dropDown_A" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">排序&nbsp;<i class="Hui-iconfont Hui-iconfont-arrow2-bottom"></i></span>
			<ul class="dropDown-menu menu radius box-shadow">
				...
				<a class="btn btn-primary radius fr"
					href="javascript:Data.edit(0,true,${parentId });"><i
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/foodCategory/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var parentName = data.result[i].parentName == null ? "" : data.result[i].parentName;
							...
							html += "<td class='col1'><a href='javascript:Data.edit(\""+id+"\",true,\""+parentId+"\");' >" + name + "</a></td>";
							html += "<td class='col1'>" + statusName + "</td>";
							html += "<td class='col1'>" + parentName + "</td>";
							html += "<td class='col1'>";
...
// 						alert(html);
		...
		"edit":function(id,isChild,parentId){
			if(!id){
				id=0;
			}
			layer.open({
		      ...
		      area: ['600px', '400px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/foodCategory/toEdit.do?id='+id+"&isChild="+isChild+"&parentId="+parentId
		    });
		},
		"changeStatus": function(id){
			if(confirm("确定更改状态？")){
				$.ajax({
					url:"/admin/foodCategory/changeStatus.do",
					data:{
						"id" : id
					},
					"dataType" : "json",
