...
<body>
	<div class="pd-20">
		<form action="/admin/coupon/list.do" method="post" id="form">
			...
			<select class="select" name="createType" id="createType" size="1">
				...
				<option value="2" <c:if test="${createType == 2}">selected</c:if>>短信预约</option>
					...
					<input type="text" placeholder="搜索角色名称" class="input-text ac_input"
						value="${searchKey}" id="searchKey" name="searchKey"
						autocomplete="off" style="width: 200px"> <a
		...
		<span class="btn btn-primary radius dropDown dropDown_hover fr"> <span class="dropDown_A" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">排序&nbsp;<i class="Hui-iconfont Hui-iconfont-arrow2-bottom"></i></span>
			<ul class="dropDown-menu menu radius box-shadow">
				...
				<a class="btn btn-primary radius fr" href="javascript:Data.edit();"><i
					class="Hui-iconfont Hui-iconfont-add"></i> 新增</a>
			</div>
		</form>
		<table
			...
			<thead>
				<tr class="text-c">
					...
					<th width="25">#</th>
					<th class="col2">优惠券名称</th>
					<th class="col2">优惠券金额</th>
					<th class="col2">有效开始日期</th>
					<th class="col2">有效结束日期</th>
					<th class="col2">数量</th>
					<th class="col2">是否删除</th>
					<th class="col2">操作</th>
				</tr>
			</thead>
			<tbody id="dataTable">
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/coupon/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var money = data.result[i].money == null ? "" : data.result[i].money;
							var startDate = data.result[i].startDate == null ? "" : data.result[i].startDate;
							var endDate = data.result[i].endDate == null ? "" : data.result[i].endDate;
							var number = data.result[i].number == null ? "" : data.result[i].number;
							var dr = data.result[i].dr == null ? "" : data.result[i].dr;
							if (startDate != null && startDate != "") {
								var value = new Date(startDate);
							...
							html += "<td class='col1'>" + money + "</td>";
							html += "<td class='col1'>" + startDate + "</td>";
							html += "<td class='col1'>" + endDate + "</td>";
							html += "<td class='col1'>" + number + "</td>";
							html += "<td class='col1'>" + dr + "</td>";
							html += "<td class='col1'>";
							if(dr==1){
								html += "删除"
							}else{
		...
		"edit":function(id){
			...
			layer.open({
		      ...
		      area: ['600px', '450px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/coupon/toEdit.do?id='+id
		    });
		},
		"delete": function(id){
			if(confirm("是否删除记录！")){
				$.ajax({
					url:"/admin/coupon/delete.do",
					data:{
						"id" : id
					},
					"dataType" : "json",
