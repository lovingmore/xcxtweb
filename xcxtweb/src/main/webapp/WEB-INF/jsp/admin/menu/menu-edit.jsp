...
<div class="page-container" >
	<form action="/admin/menu/save.do" class="form form-horizontal" method="post" id="form">
	...
	<div class="row cl">
		...
		<input datatype="*" maxlength="30" id="name" name="name" value="${menu.name }" placeholder="请输入菜单名称" class="input-text" type="text" />
		</div>
	</div>
	<div class="row cl">
		...
		<select id="parentId" name="parentId" class="input-text">
			...
			<c:forEach items="${menus }" var="mu">
				<option value="${mu.id }" <c:if test="${menu.parentId==mu.id }">selected</c:if>>${mu.name }</option>
			</c:forEach>
		</select>
		</div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-8">
		<input maxlength="30" id="listno" name="listno" value="${menu.listno}" placeholder="请输入排序序号" class="input-text" type="text" />
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-2 col-sm-2">链接地址：</label>
		<div class="formControls col-xs-8 col-sm-8">
		<input maxlength="30" id="url" name="url" value="${menu.url}" placeholder="请输入链接地址" class="input-text" type="text" />
		</div>
	</div>
	<div class="row cl">
