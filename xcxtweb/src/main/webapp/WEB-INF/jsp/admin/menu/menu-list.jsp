...
<body>
	<div class="pd-20">
		<form action="/admin/menu/list.do" method="post" id="form">
			<input type="hidden" id="sysType" name="sysType" value="1" /> <input
				type="hidden" id="orderDefind" name="orderDefind"
				value="${orderDefind}" />
			...
			<select class="select" name="createType" id="createType" size="1">
				...
				<option value="2" <c:if test="${createType == 2}">selected</c:if>>短信预约</option>
					...
					<input type="text" placeholder="搜索菜单名称" class="input-text ac_input"
		...
		<table
			...
			<thead>
				<tr class="text-c">
					...
					<th width="25">#</th>
					<th class="col2">菜单名称</th>
					<th class="col2">链接地址</th>
					<th class="col2">父菜单</th>
					<th class="col2">序号</th>
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/menu/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var url = data.result[i].url == null ? "" : data.result[i].url;
							var parentId = data.result[i].parentId == null ? "" : data.result[i].parentId;
							var listno = data.result[i].listno == null ? "" : data.result[i].listno;
							var createTime = data.result[i].createTime == null ? "" : data.result[i].createTime;
							if (createTime != null && createTime != "") {
								var value = new Date(createTime);
								createTime = value.format("yyyy-MM-dd hh:mm:ss");
							}
							html += "<tr>";
							html += "<td class='col1'>"+(i+1)+"</td>";
							html += "<td class='col1'><a href='javascript:Data.edit(\""+id+"\");' >" + name + "</a></td>";
							html += "<td class='col1'>" + url + "</td>";
							html += "<td class='col1'>" + parentId + "</td>";
							html += "<td class='col1'>" + listno + "</td>";
							html += "<td class='col1'>" + createTime + "</td>";
							html += "</tr>";
						}
// 						alert(html);
						$("#dataTable").html(html);
		...
		"edit":function(id){
			...
			layer.open({
		      ...
		      area: ['600px', '400px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/menu/toEdit.do?id='+id
		    });
		},
		"delete": function(){
			if(isSelected(1)){
