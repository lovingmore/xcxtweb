...
<body>
	<div class="pd-20">
		<form action="/admin/user/list.do" method="post" id="form">
			...
			<select class="select" name="createType" id="createType" size="1">
				...
				<option value="2" <c:if test="${createType == 2}">selected</c:if>>短信预约</option>
					...
					<input type="text" placeholder="搜索账号，姓名"
						class="input-text ac_input" value="${searchKey}" id="searchKey"
						name="searchKey" autocomplete="off" style="width: 200px">
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/user/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data != "" && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var username = data.result[i].username == null ? "" : data.result[i].username;
							...
							html += "<td class='col1'><a href='javascript:Data.edit(\""+id+"\");' >" + username + "</a></td>";
							html += "<td class='col1'>" + name + "</td>";
							html += "<td class='col1'>" + name + "</td>";
							html += "<td class='col1'>" + createTime + "</td>";
							html += "<td class='col1'>";
							html += "<a href='javascript:Data.initPwd(\""+id+"\");' >初始密码</a> ";
							html += "<a href='javascript:Data.delete(\""+id+"\");' >删除用户</a> ";
							html += "</td>";
							html += "</tr>";
						}
// 						alert(html);
		...
		"edit":function(id){
			...
			layer.open({
		      ...
		      area: ['600px', '350px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/user/toEdit.do?id='+id
		    });
		},
		"initPwd":function(id){
			if(window.confirm("确定要初始化密码？")){
				$.ajax({
					url:"/admin/user/initPwd.do",
					data:{
						"id" : id
					},
					"dataType" : "json",
