...
<body>
	<div class="page-container">
		<form action="/admin/user/save.do" class="form form-horizontal"
			method="post" id="form">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8">
					<input datatype="*" maxlength="30" id="username" name="username"
						value="${user.username }" placeholder="请输入账号" class="input-text"
						type="text" />
				</div>
			</div>
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8">
					<input datatype="*" maxlength="30" id="name" name="name"
						value="${user.name }" placeholder="请输入用户名称" class="input-text"
						type="text" />
				</div>
			</div>
			<div class="row cl">
					...
					<select id="roleId" name="roleId" class="input-text">
						...
						<c:forEach items="${roles }" var="role">
							<option value="${role.id }"
								<c:if test="${!empty user.roleId && user.roleId==role.id }">selected</c:if>>${role.name }</option>
						</c:forEach>
					</select>
				</div>
			</div>
