...
<body>
<div class="pd-20">
	<form action="/admin/customer/list.do" method="post" id="form">
	...
	<div class="cl pd-5 bg-1 bk-gray">
		...
		<input type="text" placeholder="搜索店名、地址、负责人" class="input-text ac_input" value="${searchKey}" id="searchKey" name="searchKey" autocomplete="off" style="width:200px">
	...
	<thead>
	  <tr class="text-c">
		...
		<th class="col2">店标</th>
		<th class="col2">账号</th>
		<th class="col2">门店名称</th>
		<th class="col2">负责人</th>
		<th class="col2">门店地址</th>
		<th class="col2">下单次数</th>
		<th class="col2">购买总金额</th>
		<th class="col2">收货地址</th>
		<th class="col2">操作</th>
	  </tr>
    </thead>
	<tbody id="dataTable">
...
<script type="text/javascript">
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/customer/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var shopbrand = data.result[i].shopBrand == null ? "" : data.result[i].shopBrand;
							var useraccount = data.result[i].userAccount == null ? "" : data.result[i].userAccount;
							var shopname = data.result[i].shopName == null ? "" : data.result[i].shopName;
							var name = data.result[i].name == null ? "" : data.result[i].name;
							var shopaddr = data.result[i].shopAddr == null ? "" : data.result[i].shopAddr;
							var ordernum = data.result[i].orderNum == null ? "" : data.result[i].orderNum;
							var orderamount = data.result[i].orderAmount == null ? "" : data.result[i].orderAmount;
							
							html += "<tr>";
							html += "<td class='col1'>"+(i+1)+"</td>";
							html += "<td class='col1'><img width='50px' height='50px' src='/images/cache/" + shopbrand + "'/></td>";
							html += "<td class='col1' align='center'>" + useraccount + "</td>";
							html += "<td class='col1'>" + shopname + "</td>";
							html += "<td class='col1'>" + name + "</td>";
							html += "<td class='col1'>" + shopaddr + "</td>";
							html += "<td class='col1'>" + ordernum + "</td>";
							html += "<td class='col1'>" + orderamount + "</td>";
							html += "<td class='col1' align='center'>";
							html += "<a href='javascript:Data.viewaddrlist(\""+id+"\");' >查看</a></td>";
							html += "<td class='col1' align='center'>";
							html += "<a href='javascript:Data.edit(\""+id+"\");' >编辑</a></td>";
							html += "</tr>";
						}
// 						alert(html);
						$("#dataTable").html(html);
		...
		"edit":function(id){
			...
			var layerIndex = layer.open({
		      ...
		      area: ['650px', '500px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/customer/toEdit.do?id='+id
		    });
			layer.full(layerIndex);
		},
		"viewaddrlist":function(id){
			if(!id){
				id=0;
			}
			var layerIndex = layer.open({
		      ...
		      area: ['650px', '500px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/recaddr/toViewaddrList.do?id='+id
		    });
			layer.full(layerIndex);
		},
		...
		"changeStatus":function(id){
			if(confirm("确定更改状态？")){
				$.ajax({
					url:"/admin/customer/changeStatus.do",
					data:{
						"id" : id
					},
					"dataType" : "json",
