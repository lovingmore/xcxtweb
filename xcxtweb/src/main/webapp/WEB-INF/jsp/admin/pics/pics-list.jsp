...
<body>
	<div class="pd-20">
		<form action="/admin/pics/list.do" method="post" id="form">
			...
			<select class="select" name="createType" id="createType" size="1">
				...
				<option value="2" <c:if test="${createType == 2}">selected</c:if>>短信预约</option>
					...
					<input type="text" placeholder="搜索标题" class="input-text ac_input"
	...
	var Data = {
		"get" : function(pageId,searchKey){
			...
			var url = "/admin/pics/listAjax.do";
			$.ajax({
				"url" : url,
				"data" : {
					"pageId" : pageId,
				...
				"success" : function(data) {
					if(data && data.status==1){
						...
						for(var i = 0; i<data.result.length; i++){
							...
							var title = data.result[i].title == null ? "" : data.result[i].title;
							var type = data.result[i].type == null ? "" : data.result[i].type;
							...
							html += "<td class='col1'><a href='javascript:Data.edit(\""+id+"\");' >" + title + "</a></td>";
							html += "<td class='col1'>" + type + "</td>";
							html += "<td class='col1'><img src='"+url+"' width='100px'></td>";
		...
		"edit":function(id){
			...
			layer.open({
		      ...
		      area: ['600px', '350px'],
		        ...
		        maxmin: true,
		        shade: false,
		      content: '/admin/pics/toEdit.do?id='+id
		    });
		},
		"delete": function(id){
			if(confirm("确定删除记录？")){
				$.ajax({
					url:"/admin/pics/delete.do",
					data:{
						"id" : id
					},
					"dataType" : "json",
